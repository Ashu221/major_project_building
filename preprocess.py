...
def update(num, X_buf, Y_buf, line):
	line.set_data(Y_buf[:num], X_buf[:num])
	line.axes.axis([60, 180, 140, 260])
	return line,

ani = animation.FuncAnimation(fig, update, len(X_buf), fargs=[X_buf, Y_buf, line], interval = 1, blit=True)
plt.show()
